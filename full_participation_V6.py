...
p_EV = pd.read_csv('./p_EVV5.csv',sep=",",encoding = "ISO-8859-1", header=0, index_col=False)#
...
for year in range(2018-2018,2050+1-2018):
    yr = p_EV.columns[year]
    d_neg = p_EV[p_EV[yr]<0]
    print(yr,len(d_neg))
...
d_WM=d_WM.drop(['hour','nutscode','TIME','country','h','month','Unnamed: 0'],axis=1) # remove redundant variable
...
d_WH=d_WH.drop(['hour','nutscode','TIME','country','Unnamed: 0'],axis=1) # remove redundant variable
...
d_TD=d_TD.drop(['hour','nutscode','TIME','country','Unnamed: 0'],axis=1) # remove redundant variable
...
d_SH=d_SH.drop(['hour','nutscode','TIME','country','Unnamed: 0'],axis=1) # remove redundant variable
...
d_RF=d_RF.drop(['hour','nutscode','TIME','country','Unnamed: 0'],axis=1) # remove redundant variable
d_RF=d_FR.drop(['hour','nutscode','TIME','country','Unnamed: 0'],axis=1) # remove redundant variable
d_RF_FR = d_RF+d_FR
...
d_RF_FR=d_RF_FR.drop(['hour','nutscode','TIME','country','Unnamed: 0'],axis=1) # remove redundant variable
del d_RF
del d_FR
...
d_EV=d_EV.drop(['hour','nutscode','TIME','country','Unnamed: 0'],axis=1) # remove redundant variable
...
d_DW=d_DW.drop(['hour','nutscode','TIME','country','Unnamed: 0'],axis=1) # remove redundant variable
...
d_AC=d_AC.drop(['hour','nutscode','TIME','country','Unnamed: 0'],axis=1) # remove redundant variable
...
d_CP=d_CP.drop(['hour','nutscode','TIME','country','Unnamed: 0'],axis=1) # remove redundant variable
...
d_HP = d_HP.drop(['hour','nutscode','TIME','country','Unnamed: 0'],axis=1) # remove redundant variable
...
final.to_csv(r'./Full_potential.V6.csv', index = False)
...
for i in range(0,final.shape[0]):
    print(i)
    final['country'][i] = final.region[i][0:2]
...
#final_country.to_csv(r'./Full_potential.V5.country.csv', index = False)
#df = pd.read_csv('I:/Projekte/OpenEntrance - WV0173/Durchführungsphase/WP6/CS1/OE_data_analysis/openEntrance/data/theoretical potential/Full_potential.V5.country.csv',sep=",",encoding = "ISO-8859-1", header=0, index_col=False)#
...
final_country.to_csv(r'./Full_potential.V6.country.csv', index = False)


# aggregate all countries
final_agg = final_country.groupby(['subannual','variable'], as_index = False).sum()
final_agg.to_csv(r'./Full_potential.V6.Agg.csv', index = False)
"""
